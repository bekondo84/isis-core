  ---Create un sequence
  --CREATE SEQUENCE seq_seqnumber START WITH 1 INCREMENT BY 1;
  ...
  CREATE TABLE IF NOT EXISTS adm_catalog (
      ...
      seqnumber BIGINT GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE IF NOT EXISTS adm_plugin_category (
    ...
    title VARCHAR (255),
     ...
     seqnumber BIGINT GENERATED BY DEFAULT AS IDENTITY,
     stringvalue TEXT DEFAULT '',
    CONSTRAINT pk_adm_plugin_category PRIMARY KEY (code)
);
-- Create Plugin Table -----
CREATE TABLE IF NOT EXISTS  adm_plugin (
   ...
   CONSTRAINT fk_adm_plugin_category FOREIGN KEY (category)
   REFERENCES adm_plugin_category (code)
  );
...
CREATE TABLE IF NOT EXISTS bs_user (
   ...
   CONSTRAINT fk_bs_user_business FOREIGN KEY (business)
   REFERENCES bs_entity (code)
);
----UserGroup ----------------
CREATE TABLE IF NOT EXISTS bs_usergroup (
    ...
    CONSTRAINT pk_bs_usergroup PRIMARY KEY (code),
    CONSTRAINT fk_bs_usergroup_plugin FOREIGN KEY (plugin_id, plugin_version)
     REFERENCES adm_plugin (id, version)
);
--Join table between bs_user and bs_usergroup ----------
CREATE TABLE IF NOT EXISTS bs_user_group (
    ug_code VARCHAR (50) NOT NULL,
    us_code VARCHAR (255) NOT NULL,

    CONSTRAINT pk_bs_user_group PRIMARY KEY (us_code, ug_code),
    CONSTRAINT fk_bs_user FOREIGN KEY (us_code) REFERENCES bs_user,
    CONSTRAINT fk_bs_usergroup FOREIGN KEY (ug_code) REFERENCES bs_usergroup
);
----UserRigth ------------------
CREATE TABLE IF NOT EXISTS bs_rigth (
    ...
    CONSTRAINT fk_bs_rigth_usergroup FOREIGN KEY (ug_code)
    REFERENCES bs_usergroup (code)
);
-------Settings tables -----------
CREATE TABLE IF NOT EXISTS adm_settings (
   ...
   createdAt TIMESTAMPTZ NOT NULL,
   lastModif TIMESTAMPTZ,
   CONSTRAINT pk_adm_settings PRIMARY KEY (key)
);