...
public class MetaDataServiceImpl implements MetaDataService {
    ...
    public MetaData buildMetaDataFrom(Class clazz, PluginWrapper wrapper, String templatename, Map<String, MetaData> createMeta, List<String> allReadyProcess, boolean principal) throws JAXBException {
        ...
        //System.out.println(String.format("-------%s-----------------%s----------------------------  %s : %s",clazz, wrapper, templatename, template));
    ...
    private void setPageSize(MetaData metaData) {
        try {
            SettingsModel settings = flexibleSearch.find(new SettingsModel("PAGE_SIZE"));
            metaData.setPageSize(Short.valueOf(settings.getValue()));
        } catch (Exception e) {
            metaData.setPageSize(Short.valueOf("20"));
        }
    }
    ...
    private void buildDashboard(PluginWrapper wrapper, Context template, MetaData metaData) {
        ...
        dashboardMeta.setTitle(i18NService.getMessage(wrapper, dashbord.getName()));

        if (CollectionUtils.isEmpty(dashbord.getSection()))
             return;

        for (DashboardSectionType sectionType : dashbord.getSection()) {
            SectionData sectionData = new SectionData(sectionType.getName(), i18NService.getMessage(wrapper, sectionType.getName()), sectionType.getColumns().intValue(), sectionType.isHeader());
            ...
            //Process SubType
             ...
             for (DashboardSubSectionType subSectionType : sectionType.getSection()) {
                  SectionData subSectionData = new SectionData(subSectionType.getName(), i18NService.getMessage(wrapper, subSectionType.getName()), subSectionType.getColumns().intValue(), subSectionType.isHeader());
    ...
    private void processDashbordFields(PluginWrapper wrapper, List<DashboardItem> items, SectionData sectionData) {
        for (DashboardItem dashboardItem : items) {
            ...
            column.setLabel(i18NService.getMessage(wrapper, dashboardItem.getName()));
    ...
    private void buildEditorArea(PluginWrapper wrapper, Context template, MetaData meta, List<Field> fields, Map<String, MetaData> createMeta, List<String> allReadyProcess, boolean principal) throws JAXBException {
        ...
        //add editor title
        meta.setFormViewTitle(i18NService.getMessage(wrapper, meta.getTypeCode().concat(".form.title")));
    ...
    private void buildForm(PluginWrapper wrapper, MetaData meta, FormData formData, FormType formType, List<Field> fields, Map<String, MetaData> createMeta, List<String> allReadyProcess, boolean principal) throws JAXBException {
        ...
        formType.getSection().sort(new Comparator<SectionType>() {
            @Override
            public int compare(SectionType o1, SectionType o2) {
    ...
    private static void sortSections(SectionType sectionType) {
        sectionType.getSection().sort(new Comparator<SubSectionType>() {
            @Override
            public int compare(SubSectionType o1, SubSectionType o2) {
                return Long.compare(o1.getPosition(), o2.getPosition());
            }
        });
    }
    ...
    private void buildSubsectionType(PluginWrapper wrapper, MetaData meta, SectionData sectionData, SubSectionType sectionType, List<Field> fields, Map<String, MetaData> createMeta, List<String> allReadyProcess, boolean principal) throws JAXBException {
        ...
        sectionData.setTitle(i18NService.getMessage(wrapper, sectionData.getName()));
        ...
        for (Field field : managedFields) {
            ...
            if(Collection.class.isAssignableFrom(field.getType()))
                ...
                column.setType(ReflectionUtils.getGenericType(field).getName());
            ...
            if (Objects.nonNull(fieldType.getFilters())
               && !CollectionUtils.isEmpty(fieldType.getFilters().getFilter())) {
                for (FilterType filterType : fieldType.getFilters().getFilter()) {
                    if (Objects.nonNull(filterType.getOperator())) {
                        FilterData filter = new FilterData(filterType.getSource(), filterType.getValue(), filterType.getOperator().value());
                       column.addFilter(filter);
                    }//End if (Objects.nonNull(filterType.getOperator()))
                }
            }
        }
    }
    ...
    private void buildListView(PluginWrapper wrapper, Context template, MetaData meta, List<Field> fields, Map<String, MetaData> createMeta, List<String> allReadyProcess) throws JAXBException {
        ...
        //Set ListEditor Title
        meta.setListViewTitle(i18NService.getMessage(wrapper, meta.getTypeCode().concat(".list.title")));
    ...
    private String getMessage(PluginWrapper wrapper, MetaData meta, Field field) {
        return i18NService.getMessage(wrapper, meta.getTypeCode().concat(".").concat(field.getName()));
    }
    ...
    private void setComplexTypeField(PluginWrapper wrapper, Field field, MetaColumn column, Map<String, MetaData> createMeta, List<String> allReadyProcess) throws JAXBException {
        ...
        try {
              metaType = flexibleSearch.find(new MetaTypeModel(typeCode));
              fieldtemplate = metaType.getTemplate();
